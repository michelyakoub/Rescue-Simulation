...
public class Simulator implements WorldListener {
	...
	public Rescuable getRescuableByAddress(int x,int y){
		ArrayList<ResidentialBuilding> b = buildings;
		for(int i=0;i<b.size();i++){
			if(b.get(i).getLocation().getX()==x&&b.get(i).getLocation().getY()==y)
				return b.get(i);
		}
		ArrayList<Citizen> c = citizens;
		for(int i=0;i<c.size();i++){
			if(c.get(i).getLocation().getX()==x&&c.get(i).getLocation().getY()==y)
				return c.get(i);
		}
		return null;
	}
